    ...
    for epoch in range(epochs):
        ...
        for i, input in enumerate(progress_bar):
            ...
            if loss_dict:
                if loss_dict.get('loss_ctc') is not None:
                    writer.add_scalar(
                        'train/loss_ctc', loss_dict['loss_ctc'].item(), global_step)
                if loss_dict.get('loss_rnnt') is not None:
                    writer.add_scalar(
                        'train/loss_rnnt', loss_dict['loss_rnnt'].item(), global_step)
        ...
        print(
            f"Epoch {epoch+1} 完成, RNNT训练损失: {epoch_loss:.4f}, 测试损失: {test_loss:.4f}")
