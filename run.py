...
train_dataloader = get_dataloader(
    "./dataset/split/train/wav.scp", "./dataset/split/train/pinyin", 32, tokenizer, shuffle=True)
test_dataloader = get_dataloader(
    "./dataset/split/test/wav.scp", "./dataset/split/test/pinyin", 32, tokenizer, shuffle=False)
...
for epoch in range(epochs):
    ...
    print(
        f"Epoch {epoch+1} done, : train loss: {epoch_loss:.4f}, test loss: {test_loss:.4f}")

    scheduler.step(test_loss)

    writer.add_scalar('epoch/train_loss', epoch_loss, epoch)
    writer.add_scalar('epoch/test_loss', test_loss, epoch)
    writer.add_scalar('epoch/learning_rate', optim.state_dict()
